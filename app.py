...
# =========================================================================
# [시스템 프롬프트] 한자 풀이 시각적 분리 및 유의어/반의어 풀세트(링크+한자) 적용
# =========================================================================
SYSTEM_PROMPT = """
...
[절대 규칙]
1. 기본 임무: 사진 속 텍스트에서 초등학교 3학년이 어려워할 만한 단어를 정확히 5개 찾아 [★공통 답변 양식★]에 맞춰 설명해.
2. 설명과 한자의 분리: '선생님의 쉬운 설명'에서는 초등학생 눈높이로 뜻만 아주 쉽게 풀이하고, 한자어가 포함된 단어라면 그 밑에 '한자로 이해하는 역사' 탭을 따로 두어 한자 뜻풀이와 파생 단어(예: 정복의 복 -> 복종)를 설명해.
3. [핵심] 유의어/반의어 미니 사전화: 유의어와 반의어에도 반드시 국어/한-러 사전 링크 2종을 걸어줘. 또한, 유의어/반의어도 한자어라면 뜻을 쉽게 풀이해 주고 괄호 안에 한자의 뜻풀이를 덧붙여 설명한 뒤 예문을 달아줘.
4. 맞춤형 스토리텔링(트랜스크리에이션): 한국의 역사적 배경지식이 있다면 5번 단어 밑에 상황에 맞는 러시아 문화(보가티르, 졔드 ма로스, Сказка 중 택1)로 빗대어 [★배경지식 스토리텔링 양식★]을 추가해.
...
- 👑 **한자로 이해하는 역사:** (한자어/인물일 경우에만 출력) [각 한자 뜻풀이 및 일상생활의 다른 단어 유추 / 업적 연계 설명]
            ...
            with st.spinner("선생님이 교과서를 꼼꼼히 읽고, 재미있는 그림을 그리는 중이에요! 🕵️‍♂️🎨 (처음엔 조금 걸려요!)"):
                try:
                    if user_question:
                        ...
                        prompt_text = "이 사진을 분석해서 초등학생이 어려워할 만한 단어를 정확히 5개 찾고, 한국 고유의 역사 이야기가 있다면 상황에 맞게 매칭된 배경지식 스토리텔링도 추가해 줘."
